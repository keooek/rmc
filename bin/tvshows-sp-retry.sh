...
for j in $(find /media/MULTIMEDIA/TORRENT/TVSHOWS-SP -type f -size +100M) ; do
 #cd /opt/share/filebot/bin; ./filebot.sh -non-strict --log all --log-file /home/pi/bin/log/tvshowssp_rename_${date_str}.txt --lang en --format "/media/MULTIMEDIA/VIDEO/TVSHOWS-SP/{n}/T{s}/{n}-{s00e00}-{t}" -rename "$j" --conflict override --db TheTVDB -non-strict
 cd /opt/share/filebot/bin; ./filebot.sh -non-strict --log all --log-file /home/pi/bin/log/tvshowssp_rename_${date_str}.txt --lang en --format "/media/MULTIMEDIA/VIDEO/TVSHOWS-SP/{n}/T{s}/{n}-{s00e00}-{t}" -rename "$j" --conflict override --db TheMovieDB -non-strict
done
unset IFS
