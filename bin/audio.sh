...
for d in $(find $rmc_base_hd_input/AUDIO/ -mindepth 1 -maxdepth 1 -type d ! -name tmp ) ; do 
 ...
 if [ "$(find "$rmc_base_hd_input/AUDIO/tmp/" -maxdepth 1 -type d -printf 1 | wc -m)" -eq 2 -a "$(find "$rmc_base_hd_input/AUDIO/tmp/" -maxdepth 1 ! -type d -printf 1 | wc -m)" -eq 0 ]; then
  ...
  [ -d "$rmc_base_hd_audio/UNCATALOGED/$(cd $rmc_base_hd_input/AUDIO/tmp/; ls -1 *)" ] && mv $rmc_base_hd_input/AUDIO/tmp/$(cd $rmc_base_hd_input/AUDIO/tmp/; ls -1 *)/* $rmc_base_hd_audio/UNCATALOGED/$(cd $rmc_base_hd_input/AUDIO/tmp/; ls -1 *) ; rm -rf $rmc_base_hd_input/AUDIO/tmp/*$rmc_base_hd_input/AUDIO/tmp/*
 else
  rm -rf $rmc_base_hd_input/AUDIO/tmp/*
