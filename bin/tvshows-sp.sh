...
( egrep -i '(MOVE|Skipped)' $rmc_logs/tvshowssp_rename_${date_str}.txt; echo "SKIPPED"; ls -1rt $rmc_base_hd_input/SKIPPED; ) | mail -s "$(egrep -i '(MOVE|Skipped)' $rmc_logs/tvshowssp_rename_${date_str}.txt| wc -l)-$([[ $lsA ]] && echo "SK-")TV-SP $(basename "$j")" $rmc_notify_mail
