...
filebot.sh -r -rename "$base_hd_input/MOVIES-EN" -non-strict --conflict override --db TheMovieDB --lang en --output "$base_hd_output/MOVIES-EN" --log all --log-file $logs/moviesen_rename_${date_str}.txt
filebot.sh -r -script fn:cleaner "$base_hd_input/MOVIES-EN" --log all --log-file $logs/moviesen_clean_$(date +%Y%m%d-%H_%M_%S).txt
filebot.sh -r -script fn:cleaner "$base_hd_output/MOVIES-EN" --log all --log-file $logs/moviesen_clean_$(date +%Y%m%d-%H_%M_%S).txt
rm $base_hd_output/MOVIES--EN/{*.sub,*.srt,*.url,*.idx,*.nfo,*.rar}
#filebot.sh -script fn:artwork.tmdb "$base_hd_output/MOVIES-EN"
