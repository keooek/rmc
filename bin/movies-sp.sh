...
filebot.sh -r -rename "$base_hd_input/MOVIES-SP" -non-strict --conflict override --db TheMovieDB --lang es --output "$base_hd_output/MOVIES-SP" --log all --log-file $logs/moviessp_rename_${date_str}.txt
filebot.sh -r -script fn:cleaner "$base_hd_input/MOVIES-SP" --log all --log-file $logs/moviessp_clean_$(date +%Y%m%d-%H_%M_%S).txt
filebot.sh -r -script fn:cleaner "$base_hd_output/MOVIES-SP" --log all --log-file $logs/moviessp_clean_$(date +%Y%m%d-%H_%M_%S).txt
rm $base_hd_output/MOVIES-SP/{*.sub,*.srt,*.url,*.idx,*.nfo,*.rar}
#filebot.sh -script fn:artwork.tmdb "$base_hd_output/MOVIES-SP"
