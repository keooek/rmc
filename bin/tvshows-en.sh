...
for j in $(find $base_hd_input/TVSHOWS-EN -type f -size +100M) ; do
 filebot.sh -non-strict --log all --log-file $logs/tvshowsen_rename_${date_str}.txt --lang en --format "$base_hd_output/TVSHOWS-EN/{n}/T{s}/{n}-{s00e00}-{t}" -rename "$j" --conflict override --db TheTVDB
done
unset IFS

filebot.sh -r -script fn:cleaner "$base_hd_input/TVSHOWS-EN" --log all --log-file $logs/tvshowsen_clean_$(date +%Y%m%d-%H_%M_%S).txt
filebot.sh -r -script fn:cleaner "$base_hd_output/TVSHOWS-EN" --log all --log-file $logs/tvshowsen_clean_library_$(date +%Y%m%d-%H_%M_%S).txt
