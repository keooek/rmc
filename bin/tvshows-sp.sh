...
for j in $(find $base_hd_input/TVSHOWS-SP -type f -size +100M) ; do
 cd /opt/share/filebot/bin; ./filebot.sh -non-strict --log all --log-file $logs/tvshowssp_rename_${date_str}.txt --lang es --format "$base_hd_output/TVSHOWS-SP/{n}/T{s}/{n}-{s00e00}-{t}" -rename "$j" --conflict override --db TheTVDB -non-strict
done
unset IFS
