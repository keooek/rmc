...
for j in $(find $rmc_base_hd_input/TVSHOWS-SP -type f -size +100M) ; do
 filebot.sh -non-strict --log all --log-file $rmc_logs/tvshowssp_rename_${date_str}.txt --lang es --format "$rmc_base_hd_output/TVSHOWS-SP/{n}/T{s}/{n}-{s00e00}-{t}" -rename "$j" --conflict override --db TheTVDB -non-strict
done
unset IFS
...
filebot.sh -r -script fn:cleaner "$rmc_base_hd_output/TVSHOWS-SP" --log all --log-file $rmc_logs/tvshowssp_clean_library_$(date +%Y%m%d-%H_%M_%S).txt
